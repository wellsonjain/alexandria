...
class EmbedPicker
  ...
  # 1. 如果 params[:embed] 為空則回傳空陣列
  # 2. split 後分別進行 valiation
  def validate_embeds
  ...
  def error!(embed)
    raise RepresentationBuilderError.new("embed=#{embed}"), "Invalid Embed. Allowed relations: (#{@presenter.class.relations.join(', ')})"
  end
